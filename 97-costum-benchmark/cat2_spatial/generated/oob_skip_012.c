 ...
 * PURPOSE: OOB from 128-byte obj at offset 127 → adjacent 128B object
 *
...
int main(void) {
    ...
    if (raw_dist > 0 && raw_dist < 100000) {
        volatile char c = a[raw_dist + 127];
        printf("a[%ld]=%02x\n", (long)(raw_dist + 127), (unsigned char)c);
    } else {
