 ...
 * PURPOSE: OOB from 1024-byte obj at offset 1023 → adjacent 1024B object
 *
...
int main(void) {
    ...
    if (!a || !b) return 1;
    memset(a, 0xAA, 1024);
    memset(b, 0xBB, 1024 > 256 ? 256 : 1024);
    ...
    if (raw_dist > 0 && raw_dist < 100000) {
        volatile char c = a[raw_dist + 1023];
        printf("a[%ld]=%02x\n", (long)(raw_dist + 1023), (unsigned char)c);
    } else {
