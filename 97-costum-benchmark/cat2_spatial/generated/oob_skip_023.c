 ...
 * PURPOSE: OOB from 2048-byte obj at offset 1024 → adjacent 2048B object
 *
...
int main(void) {
    ...
    if (!a || !b) return 1;
    memset(a, 0xAA, 2048);
    memset(b, 0xBB, 2048 > 256 ? 256 : 2048);
    ...
    if (raw_dist > 0 && raw_dist < 100000) {
        volatile char c = a[raw_dist + 1024];
        printf("a[%ld]=%02x\n", (long)(raw_dist + 1024), (unsigned char)c);
    } else {
