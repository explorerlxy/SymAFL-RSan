 ...
 * PURPOSE: OOB from 8192-byte obj at offset 4096 → adjacent 8192B object
 *
...
int main(void) {
    char *a = (char*)malloc(8192);
    char *b = (char*)malloc(8192);
    
    if (!a || !b) return 1;
    memset(a, 0xAA, 8192);
    memset(b, 0xBB, 8192 > 256 ? 256 : 8192);
    ...
    if (raw_dist > 0 && raw_dist < 100000) {
        volatile char c = a[raw_dist + 4096];
        printf("a[%ld]=%02x\n", (long)(raw_dist + 4096), (unsigned char)c);
    } else {
