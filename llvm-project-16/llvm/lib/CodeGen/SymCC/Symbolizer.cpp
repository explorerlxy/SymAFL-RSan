...
void Symbolizer::handleFunctionCall(CallBase &I, Instruction *returnPoint) {
  ...
  // Per-call-site redirection: if the callee is an intercepted function
  // (e.g., malloc, read, memcpy), redirect this specific call to its
  // _symbolized wrapper. This is done here (not globally in
  // instrumentModule) so that __noinstrument_* and other skipped functions
  // continue calling the original function.
  if (callee != nullptr && isInterceptedFunction(*callee)) {
    ...
    FunctionCallee symbolCallee =
        I.getModule()->getOrInsertFunction(symbolName,
